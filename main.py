...
def make_page(title, content, nav="recruit", extra_css="", user=None):
    ...
    nav_links = [
        ("/", "\U0001f3ed", "招聘", "recruit"),
        ("/ai-match", "\U0001f916", "AI匹配", "aimatch"),
        ("/resumes", "\U0001f465", "简历", "resumes"),
        ("/feedback", "\U0001f4ac", "反馈", "feedback"),
    ]
    nav_html = ""
    ...
    nav_html += '<a href="/enterprise/login" style="font-size:10px;">\U0001f3e2</a>'
    return (
...
async def job_detail(request: Request, job_id: int):
    ...
    if not user_info:
        desc_display = re.sub(r'1[3-9]\d{9}', r'🔒****隐藏****', desc_display)
        desc_display = re.sub(r'【联系电话[^】]*】\s*[^\n]*', '🔒 联系方式已隐藏，登录后可查看', desc_display)
    ...
    if user_info:
        ...
        content += f"""
            <div style="text-align:center;padding:12px;background:var(--card2);border-radius:8px;">
                ...
                <div style="font-size:13px;color:var(--text2);margin-bottom:8px;">联系方式已隐藏，登录后可查看</div>
                <a href="/user/login" class="btn" style="display:inline-block;margin-bottom:4px;">🔑 登录查看</a>
